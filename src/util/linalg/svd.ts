 ...
 * https://github.com/ampl/gsl/blob/master/linalg/svd.c#L761-L943 and the last
 * lines of https://jamesmccaffreyblog.com/2024/01/25/singular-value-
 * decomposition-svd-from-scratch-using-python/
 ...
 * shape [m, n], S has length min(m, n), and V has shape [n, n]
 */
export function svd(matrix: Matrix): {
    ...
    return M < N ? {
        u: new Matrix(...Array.from(A.columns()).slice(0, M)).transpose(),
        s: new Vector(...Array.from(S).slice(0, M)),
        v: new Matrix(...Array.from(Q.columns()).slice(0, M)).transpose(),
    } : {
        u: A,
        v: Q,
        s: S,
